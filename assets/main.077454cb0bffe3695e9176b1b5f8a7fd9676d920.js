...
window.addEventListener("DOMContentLoaded", () => {
	if (document.querySelector(".toc-container")) {
		...
		const updateTocHighlighting = throttle(() => {
			const headingsCSS = Array(6).fill().map((_e, i) => i + 1).map((i) => `.body h${i}`).join(",");
			const headings = [...document.querySelectorAll(headingsCSS)];
			const tocContainers = [...document.querySelectorAll(".toc-container")];
			tocContainers.forEach((tocContainer) => {
				const tocElements = [...tocContainer.querySelectorAll(":scope > a")];
				const last = headings
					.filter((heading) => heading.getBoundingClientRect().y <= document.documentElement.clientHeight / 2)
					.filter((heading) => tocElements.some((tocElement) => tocElement.getAttribute("href") === `#${heading.id}`))
					.slice(-1)[0];
				tocElements.forEach((tocElement) => tocElement.classList.remove("active"));
				if (last !== undefined) {
					const headingId = last.id;
					const foundElement = tocElements.find((tocElement) => tocElement.getAttribute("href") === `#${headingId}`);
					if (foundElement) {
		...
		document.addEventListener("scroll", updateTocHighlighting);
		window.addEventListener("resize", updateTocHighlighting);
...
document.addEventListener("DOMContentLoaded", () => {
	[...document.querySelectorAll(".keep-scrolltop")].forEach((keepScrolltopElement, i) => {
		const prevScrollPos = sessionStorage.getItem(`scrolltop-${i}`);
		if (prevScrollPos) {
			keepScrolltopElement.scrollTo({top: prevScrollPos, behavior: "auto"});
			sessionStorage.removeItem(`scrolltop-${i}`);
		}
	});
});
...
window.addEventListener("beforeunload", () => {
	[...document.querySelectorAll(".keep-scrolltop")].forEach((keepScrolltopElement, i) => {
		sessionStorage.setItem(`scrolltop-${i}`, keepScrolltopElement.scrollTop);
	});
});
...
window.addEventListener("DOMContentLoaded", () => {
	...
	[...document.querySelectorAll("*[data-bs-hide-end-controls=\"true\"]")].forEach((carousel) => {
		const numItems = [...carousel.querySelectorAll(".carousel-item")].length;
		const updateControlVisibility = (currentIndex) => {
			carousel.querySelector("*[data-bs-slide=\"prev\"]").style.visibility = currentIndex === 0 ? "hidden" : "visible";
			carousel.querySelector("*[data-bs-slide=\"next\"]").style.visibility = currentIndex === numItems - 1 ? "hidden" : "visible";
		};
		carousel.addEventListener("slide.bs.carousel", (event) => updateControlVisibility(event.to));
		updateControlVisibility(numItems - 1);
	});
});
